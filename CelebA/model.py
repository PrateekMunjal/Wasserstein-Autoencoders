...
def encoder(X,isTrainable=True,reuse=False,name='phi_encoder'):
    with tf.variable_scope(name) as scope:
        ...
        conv1 = tf.layers.conv2d(X,kernel_initializer=tf.truncated_normal_initializer(stddev=stddev),filters=128,kernel_size=[5,5],padding='SAME',strides=(2,2),name='enc_conv1_layer',activation=None,trainable=isTrainable,reuse=reuse); 
        ...
        #32x32x128
        conv2 = tf.layers.conv2d(conv1,kernel_initializer=tf.truncated_normal_initializer(stddev=stddev),filters=256,kernel_size=[5,5],padding='SAME',strides=(2,2),name='enc_conv2_layer',activation=None,trainable=isTrainable,reuse=reuse); 
        ...
        #16x16x256
        conv3 = tf.layers.conv2d(conv2,kernel_initializer=tf.truncated_normal_initializer(stddev=stddev),filters=512,kernel_size=[5,5],padding='SAME',strides=(2,2),name='enc_conv3_layer',activation=None,trainable=isTrainable,reuse=reuse); 
        ...
        #8x8x512
        conv4 = tf.layers.conv2d(conv3,kernel_initializer=tf.truncated_normal_initializer(stddev=stddev),filters=1024,kernel_size=[5,5],padding='SAME',strides=(2,2),name='enc_conv4_layer',activation=None,trainable=isTrainable,reuse=reuse); 
...
def generator(z_sample,isTrainable=True,reuse=False,name='theta_generator'):
    with tf.variable_scope(name) as scope:  
        ...
        z_sample = tf.layers.dense(z_sample,8*8*1024,activation=None,trainable=isTrainable,reuse=reuse,name='dec_dense_fc_first_layer',kernel_initializer=tf.truncated_normal_initializer(stddev=stddev));
        #z_sample = tf.layers.batch_normalization(z_sample,training=isTrainable,reuse=reuse,name='bn_0');
        #z_sample = tf.nn.relu(z_sample);
        z_sample = tf.reshape(z_sample,[-1,8,8,1024]);
        #8x8x1024

        deconv1 = tf.layers.conv2d_transpose(z_sample,kernel_initializer=tf.random_normal_initializer(stddev=stddev),filters=512,kernel_size=[5,5],padding='SAME',activation=None,strides=(2,2),name='dec_deconv1_layer',trainable=isTrainable,reuse=reuse); # 16x16
        ...
        deconv4 = tf.nn.tanh(deconv4,name='relu_deconv_4');
